...
- 9 个脚本，仓库根目录平铺，命名统一为 `<步骤号>-<功能>.sh`：01-update / 02-tools / 04-user / 05-ufw / 06-ssh / 07-fail2ban / 08-swap（swap.sh 上游拷贝改名）/ 09-bbr / 12-verify。
- 每个脚本自包含、幂等、中文交互（08-swap.sh 上游风格），单独 raw 拉取即可执行。
- 人是编排器：README 提供命令与顺序。
...
5. 颜色：`[[ -t 1 ]]` 时定义（08-swap.sh 上游同款），否则全为空
...
- **必装清单**（与 README 第 2 步一致）：`sudo ca-certificates curl wget gnupg ufw fail2ban unattended-upgrades vim nano unzip htop git`（vim/nano 二选一：conf 里 `TOOLS_EDITOR`？——**否**，两个都装，成本为零，脚本内提示）。git 按此前决定：要用再装 → **不含 git**，需要者写进 `TOOLS_EXTRA`。
- **幂等**：apt install 天然幂等。
- **完成输出**：打印已装/跳过摘要。
...
- **产出**：终端绿/红/黄报告；写 `/root/vps-init-report.md`（含 conf 汇总：端口 / 用户 / 放行端口 / fail2ban 参数，对应流程第 13 步归档）。
- **退出码**：有红 → 1，否则 0。
...
3. 真实验收（在 Snapshot 保护的 VPS 上）：README 命令速查全流程走一遍 → `12-verify.sh` 全绿 → 人工确认新终端新端口密钥登录、root 被拒
4. `08-swap.sh` 是上游拷贝不动；`09-bbr.sh` 已入库（参数集由用户提供，bbr + fq + TCP 调优，幂等写入 /etc/sysctl.d/99-bbr.conf）
